...
import { createQueryBuilder, filterOps, exprOps, updateOps, stageHandlers, validate as Validate } from './index.js';
...
// Quote MySQL identifiers that collide with reserved words (e.g. a column
// named `longText` lexes as LONGTEXT and must be written as `longText`).
// Falls back to the raw name when Validate rejects it, preserving the
// previous behaviour of letting the database report the error.
const qi = (name, database) => {
    if (database !== 'mysql') return name;
    try { return Validate.col(name, database); } catch { return name; }
};
...
const inferSQLType = (value, database) => {
    const types = {
        ...
        mysql: { string: (v) => (typeof v === 'string' && v.length > 255 ? 'TEXT' : 'VARCHAR(255)'), number: (v) => Number.isInteger(v) ? 'INT' : 'DECIMAL(20,6)', boolean: 'TINYINT(1)', date: 'DATETIME', json: 'JSON' },
...
// Compare types ignoring any (length) suffix: information_schema reports
// MySQL VARCHAR as bare 'varchar' while our inferred types carry a length,
// so 'VARCHAR' and 'VARCHAR(255)' must be treated as the same column type.
const baseType = (type) => String(type || '').replace(/\(.*\)/, '').toUpperCase();
...
class SQLCollection {
    ...
    async createTable(document) {
        ...
        keys.forEach((key) => { if (key === '_id') return; const schemaDef = this.schema[key]; const value = document[key]; const type = (typeof schemaDef === 'string' ? schemaDef : schemaDef?.type) || inferSQLType(value, this.database); const nullable = schemaDef?.required ? ' NOT NULL' : ''; const unique = schemaDef?.unique ? ' UNIQUE' : ''; const defaultValue = schemaDef?.default !== undefined ? ` DEFAULT ${typeof schemaDef.default === 'function' ? escapeValue(schemaDef.default(), this.database) : escapeValue(schemaDef.default, this.database)}` : ''; columns.push(`${qi(key, this.database)} ${type}${nullable}${unique}${defaultValue}`); });
        const idCol = this.options.idColumn; let idColumn; if (this.options.idStrategy === 'auto') { idColumn = this.database === 'pg' ? `${idCol} SERIAL PRIMARY KEY` : this.database === 'mysql' ? `${qi(idCol, this.database)} INT AUTO_INCREMENT PRIMARY KEY` : `${idCol} INTEGER PRIMARY KEY AUTOINCREMENT`; } else { const idType = this.database === 'pg' ? 'TEXT' : this.database === 'mysql' ? 'VARCHAR(24)' : 'TEXT'; idColumn = `${qi(idCol, this.database)} ${idType} PRIMARY KEY`; }
    ...
    classifyError(e) {
        ...
        // Check missing COLUMN first: Postgres reports missing columns as
        // 'column "x" ... does not exist' (code 42703), which would otherwise
        // satisfy the generic missing-TABLE 'does not exist' match below.
        const missingColumn = code === '42703' || errno === 1054 || msg.includes('no such column') || msg.includes('unknown column');
        const missingTable = !missingColumn && (code === '42P01' || errno === 1146 || msg.includes('no such table') || msg.includes('does not exist') || msg.includes('unknown table'));
    ...
    async addMissingRenameColumns(update) { const renames = update?.$rename; if (!renames || typeof renames !== 'object' || !this.options.migrateOnUpdate) return; for (const [oldField, newField] of Object.entries(renames)) { if (typeof newField !== 'string' || oldField.includes('.') || newField.includes('.') || newField === oldField) continue; if (this.tableSchema?.columns?.[newField]) continue; const fallback = this.database === 'pg' ? 'TEXT' : this.database === 'mysql' ? 'VARCHAR(255)' : 'TEXT'; let type = this.tableSchema?.columns?.[oldField] || fallback; if (this.database === 'mysql' && /^(varchar|char|character)(\(\s*\d+\s*\))?$/i.test(type) && !/\(\s*\d+\s*\)/.test(type)) type = fallback; const addKw = 'ADD COLUMN'; try { await this.adapter.execute(`ALTER TABLE ${qi(this.name, this.database)} ${addKw} ${qi(newField, this.database)} ${type}`); this.tableSchema.columns[newField] = String(type).toUpperCase(); } catch (e) { console.warn(`[schemaless] could not pre-add rename column ${this.name}.${newField}:`, e?.message || e); } } }
    // Serial/AUTO_INCREMENT columns are NOT NULL; a null value must be dropped so the sequence default applies.
    // information_schema reports SERIAL as 'integer' and MySQL auto-increment as 'int', so the declared schema is the source of truth.
    isAutoIdColumn(column) { const def = this.schema?.[column]; const type = String(typeof def === 'string' ? def : def?.type || '').toUpperCase(); if (this.database === 'pg') return type.includes('SERIAL'); if (this.database === 'mysql') return type.includes('AUTO_INCREMENT'); return false; }
    stripAutoIncrements(doc) { const out = { ...doc }; const idCol = this.options.idColumn; for (const key of Object.keys(out)) { const isAutoId = key === idCol && this.options.idStrategy === 'auto'; if (!isAutoId && !this.isAutoIdColumn(key)) continue; if (out[key] === null || out[key] === undefined) delete out[key]; } return out; }
    ...
    async insertOne(document) { await this.initialize(); const doc = this.applyDefaults(document); const idCol = this.options.idColumn; const nonAuto = this.options.idStrategy !== 'auto'; if (nonAuto) { if (!doc[idCol]) doc[idCol] = this.generateId(); }
        ...
        const insert = this.stripAutoIncrements(doc);
    ...
    async updateOne(query, update) { await this.initialize(); await this.addMissingRenameColumns(update); const preCountSQL = this.adapter.buildCount(this.name, query); const preCountRes = await this.execWithDDLRetry(preCountSQL, { docForCreate: {}, allowColumnMigrate: false }); const preRow = preCountRes.rows?.[0] || preCountRes[0]; const matchedCount = Math.min(1, parseInt(preRow?.count || 0)); const sql = this.adapter.buildUpdateOne(this.name, query, update); const updDoc = {}; Object.assign(updDoc, update?.$set || {}); if (update?.$currentDate && typeof update.$currentDate === 'object') { for (const k of Object.keys(update.$currentDate)) { updDoc[k] = new Date(); } } for (const [key, val] of Object.entries(update || {})) { if (key && !key.startsWith('$')) { updDoc[key] = val; } } const result = await this.execWithDDLRetry(sql, { docForCreate: {}, docForMigrate: updDoc, allowColumnMigrate: this.options.migrateOnUpdate }); return { matchedCount, modifiedCount: result.affectedRows || result.rowCount || result.changes || 0, acknowledged: true }; }
    async updateMany(query, update) { await this.initialize(); await this.addMissingRenameColumns(update); const preCountSQL = this.adapter.buildCount(this.name, query); const preCountRes = await this.execWithDDLRetry(preCountSQL, { docForCreate: {}, allowColumnMigrate: false }); const preRow = preCountRes.rows?.[0] || preCountRes[0]; const matchedCount = parseInt(preRow?.count || 0); const sql = this.adapter.buildUpdateMany(this.name, query, update); const updDocMany = {}; Object.assign(updDocMany, update?.$set || {}); if (update?.$currentDate && typeof update.$currentDate === 'object') { for (const k of Object.keys(update.$currentDate)) { updDocMany[k] = new Date(); } } for (const [key, val] of Object.entries(update || {})) { if (key && !key.startsWith('$')) { updDocMany[key] = val; } } const result = await this.execWithDDLRetry(sql, { docForCreate: {}, docForMigrate: updDocMany, allowColumnMigrate: this.options.migrateOnUpdate }); return { matchedCount, modifiedCount: result.affectedRows || result.rowCount || result.changes || 0, acknowledged: true }; }
    ...
    async createIndex(field, options = {}) { const name = options.name || `idx_${this.name}_${field.replace(/\./g, '_')}`; const unique = options.unique ? 'UNIQUE' : ''; const type = this.database === 'pg' && options.type ? `USING ${options.type.toUpperCase()}` : ''; if (this.database === 'mysql') { const existing = await this.adapter.execute(`SHOW INDEX FROM ${qi(this.name, this.database)} WHERE Key_name = '${name}'`); const rows = existing.rows || existing; if (!rows || rows.length === 0) { await this.adapter.execute(`CREATE ${unique} INDEX ${qi(name, this.database)} ON ${qi(this.name, this.database)} (${qi(field, this.database)})`.trim()); } } else { await this.adapter.execute(`CREATE ${unique} INDEX IF NOT EXISTS ${name} ON ${this.name} ${type} (${field})`.trim()); } return { acknowledged: true }; }
    async dropColumn(column) { if (this.database === 'sqlite') throw new Error('SQLite does not support DROP COLUMN'); await this.adapter.execute(`ALTER TABLE ${qi(this.name, this.database)} DROP COLUMN ${qi(column, this.database)}`); if (this.tableSchema && this.tableSchema.columns) { delete this.tableSchema.columns[column]; } return { acknowledged: true }; }
}
...
export const createSQLAdapter = (config = {}) => {
    ...
    const adapter = {
        ...
        async dropCollection(name) { const sql = this.database === 'pg' ? `DROP TABLE IF EXISTS ${name} CASCADE` : this.database === 'mysql' ? `DROP TABLE IF EXISTS ${qi(name, this.database)}` : `DROP TABLE IF EXISTS ${name}`; await this.execute(sql); return { acknowledged: true }; },
        ...
        async addColumn(tableName, columnName, type, options = {}) { const nullable = options.required ? ' NOT NULL' : ''; const unique = options.unique ? ' UNIQUE' : ''; const def = options.default !== undefined ? ` DEFAULT ${escapeValue(typeof options.default === 'function' ? options.default() : options.default, database)}` : ''; const addKw = 'ADD COLUMN'; const sql = `ALTER TABLE ${qi(tableName, database)} ${addKw} ${qi(columnName, database)} ${type}${nullable}${unique}${def}`; await this.execute(sql); return { acknowledged: true }; },
        async renameColumn(tableName, oldName, newName) { const sql = `ALTER TABLE ${qi(tableName, database)} RENAME COLUMN ${qi(oldName, database)} TO ${qi(newName, database)}`; await this.execute(sql); return { acknowledged: true }; },
        async modifyColumn(tableName, columnName, newType, options = {}) { if (database === 'sqlite') throw new Error('SQLite does not support MODIFY COLUMN'); const nullable = options.required ? ' NOT NULL' : ''; const unique = options.unique ? ' UNIQUE' : ''; const def = options.default !== undefined ? ` DEFAULT ${escapeValue(typeof options.default === 'function' ? options.default() : options.default, database)}` : ''; const sql = database === 'pg' ? `ALTER TABLE ${tableName} ALTER COLUMN ${columnName} TYPE ${newType}` : `ALTER TABLE ${qi(tableName, database)} MODIFY COLUMN ${qi(columnName, database)} ${newType}${nullable}${unique}${def}`; await this.execute(sql); return { acknowledged: true }; },
        async dropIndex(tableName, indexName) { const sql = database === 'pg' ? `DROP INDEX IF EXISTS ${indexName}` : database === 'mysql' ? `DROP INDEX ${qi(indexName, database)} ON ${qi(tableName, database)}` : `DROP INDEX IF EXISTS ${indexName}`; await this.execute(sql); return { acknowledged: true }; },
