...
// MySQL identifiers that collide with type keywords (e.g. `longText` lexes as
// LONGTEXT) break DDL grammar; backtick-quote every identifier in MySQL DDL.
const quoteIdent = (name, database) => database === 'mysql' ? `\`${name}\`` : name;
...
class SQLCollection {
    ...
    async createTable(document) {
        ...
        keys.forEach((key) => { if (key === '_id') return; const schemaDef = this.schema[key]; const value = document[key]; const type = (typeof schemaDef === 'string' ? schemaDef : schemaDef?.type) || inferSQLType(value, this.database); const nullable = schemaDef?.required ? ' NOT NULL' : ''; const unique = schemaDef?.unique ? ' UNIQUE' : ''; const defaultValue = schemaDef?.default !== undefined ? ` DEFAULT ${typeof schemaDef.default === 'function' ? escapeValue(schemaDef.default(), this.database) : escapeValue(schemaDef.default, this.database)}` : ''; columns.push(`${quoteIdent(key, this.database)} ${type}${nullable}${unique}${defaultValue}`); });
    ...
    async dropColumn(column) { if (this.database === 'sqlite') throw new Error('SQLite does not support DROP COLUMN'); await this.adapter.execute(`ALTER TABLE ${this.name} DROP COLUMN ${quoteIdent(column, this.database)}`); if (this.tableSchema && this.tableSchema.columns) { delete this.tableSchema.columns[column]; } return { acknowledged: true }; }
}
...
export const createSQLAdapter = (config = {}) => {
    ...
    const adapter = {
        ...
        async addColumn(tableName, columnName, type, options = {}) { const nullable = options.required ? ' NOT NULL' : ''; const unique = options.unique ? ' UNIQUE' : ''; const def = options.default !== undefined ? ` DEFAULT ${escapeValue(typeof options.default === 'function' ? options.default() : options.default, database)}` : ''; const addKw = 'ADD COLUMN'; const sql = `ALTER TABLE ${tableName} ${addKw} ${quoteIdent(columnName, database)} ${type}${nullable}${unique}${def}`; await this.execute(sql); return { acknowledged: true }; },
        async renameColumn(tableName, oldName, newName) { const sql = `ALTER TABLE ${tableName} RENAME COLUMN ${quoteIdent(oldName, database)} TO ${quoteIdent(newName, database)}`; await this.execute(sql); return { acknowledged: true }; },
        async modifyColumn(tableName, columnName, newType, options = {}) { if (database === 'sqlite') throw new Error('SQLite does not support MODIFY COLUMN'); const nullable = options.required ? ' NOT NULL' : ''; const unique = options.unique ? ' UNIQUE' : ''; const def = options.default !== undefined ? ` DEFAULT ${escapeValue(typeof options.default === 'function' ? options.default() : options.default, database)}` : ''; const sql = database === 'pg' ? `ALTER TABLE ${tableName} ALTER COLUMN ${quoteIdent(columnName, database)} TYPE ${newType}` : `ALTER TABLE ${tableName} MODIFY COLUMN ${quoteIdent(columnName, database)} ${newType}${nullable}${unique}${def}`; await this.execute(sql); return { acknowledged: true }; },
