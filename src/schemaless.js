...
class SQLCollection {
    ...
    async addMissingRenameColumns(update) { const renames = update?.$rename; if (!renames || typeof renames !== 'object' || !this.options.migrateOnUpdate) return; for (const [oldField, newField] of Object.entries(renames)) { if (typeof newField !== 'string' || oldField.includes('.') || newField.includes('.') || newField === oldField) continue; if (this.tableSchema?.columns?.[newField]) continue; const fallback = this.database === 'pg' ? 'TEXT' : this.database === 'mysql' ? 'VARCHAR(255)' : 'TEXT'; const type = this.tableSchema?.columns?.[oldField] || fallback; const addKw = this.database === 'mysql' ? 'ADD' : 'ADD COLUMN'; try { await this.adapter.execute(`ALTER TABLE ${this.name} ${addKw} ${newField} ${type}`); this.tableSchema.columns[newField] = String(type).toUpperCase(); } catch (e) {} } }
