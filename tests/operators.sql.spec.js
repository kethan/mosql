...
const exprCases = [
  ...
  { title: '$divide', make: (db) => qb.expression({ $divide: ['$age', 2] }, db), expect: (db) => '(age * 1.0 / NULLIF(2, 0))' },
  ...
  { title: '$substr', make: (db) => qb.expression({ $substr: ['$name', 1, 2] }, db), expect: (db) => 'SUBSTRING(name, (1 + 1), 2)' },
  ...
  { title: '$toString', make: (db) => qb.expression({ $toString: ['$age'] }, db), expect: (db) => db==='mysql' ? 'CAST(age AS CHAR)' : 'CAST(age AS TEXT)' },
  { title: '$toInt', make: (db) => qb.expression({ $toInt: ['$age'] }, db), expect: (db) => db==='mysql' ? 'CAST(age AS SIGNED)' : 'CAST(age AS INTEGER)' },
  ...
  { title: '$toBool', make: (db) => qb.expression({ $toBool: ['$age'] }, db), expect: (db) => db==='mysql' ? 'IF(age, 1, 0)' : 'CAST(age AS BOOLEAN)' },
  { title: '$toDate', make: (db) => qb.expression({ $toDate: ['$age'] }, db), expect: (db) => db === 'sqlite' ? "datetime(age)" : db === 'mysql' ? 'CAST(age AS DATETIME)' : 'CAST(age AS TIMESTAMP)' },
